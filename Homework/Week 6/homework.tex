...
Let $f$ be defined on $[a,b]$, with $f'(x) = 0$ for all $x \in (a,b)$.
Let $x, y \in [a,b]$ with $x < y$. We know $f$ is differentiable on $(x,y)$,
so it's also continuous on $(x,y)$, and hence on $[x,y]$ by our lemma above.
By our discussion above, there's a $p \in (x,y)$ such that
\begin{align*}
    f(y) - f(x) &= (y-x)f'(p) = 0 \\
    f(y) &= f(x) \,.
\end{align*}
So $f$ has the same value over the entirety of $(a,b)$. The endpoints
must also have this value, or else they would constitute a jump
discontinuity. So we've shown that $f$ is constant.

Let $f$ be defined on $(a,b)$, with $f'(x) \geq 0$ for all $x \in (a,b)$.
