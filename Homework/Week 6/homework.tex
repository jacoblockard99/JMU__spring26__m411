...
\begin{quote}
    \itshape
    Suppose that $f$ is continuous on $[a,b]$, $f$ is differentiable at some
...
Let $f$ be continuous on $[a,b]$ and differentiable at some $x \in [a,b]$.
...
on $(a,b)$, so Problems 8.8 and 8.7 ensure $h$ is also differentiable on $(a,b)$.
...
In any case, we've shown that an $x$ with the desired property exists.
...
It follows directly from Cauchy's Mean Value Theorem
that there is an $x \in (a,b)$ such that
\begin{align*}
    [f(b) - f(a)]g'(x) = (g(b)-g(a)) f'(x) \\
    [f(b) - f(a)]\cdot 1 = (b-a) f'(x) \\
    f(b) - f(a) = (b-a) f'(x) \,,
\end{align*}
where $g$ is the identity function on $[a,b]$.

Let $f$ be continuous on $[a,b]$, with $f'(x) = 0$ for all $x \in (a,b)$.
Let $x, y \in [a,b]$ with $x < y$. Then $f$ is differentiable on $(x,y)$ and
continuous on $[x,y]$.
By our discussion above, there's a $p \in (x,y)$ such that
\begin{align*}
    f(y) - f(x) &= (y-x)f'(p) = 0 \\
    f(y) &= f(x) \,.
\end{align*}
So $f$ has some value over the entirety of $(a,b)$.
$f$ must also have this value at $a$ and $b$, or else
$f$ would have discontinuities
there. So we've shown that $f$ is constant.

Let $f$ be continuous on $[a,b]$, with $f'(x) \geq 0$ for all $x \in (a,b)$.
Let $x, y \in (a,b)$ with $x < y$. By our discussion above, there's a $p \in (x,y)$
such that
\begin{gather*}
    f(y) - f(x) = (y-x)f'(p) \geq 0 \\
    f(y) \geq f(x) \,.
\end{gather*}
Suppose $f(a) > f(p)$ for some $p \in (a,b)$. Then the intermediate
value theorem ensures there's a $c \in (a, p) \subseteq (a,b)$ such that $f(p) < f(c) < f(p)$.
But we've already shown $f(c) \geq f(p)$, so we conclude $f(a) \leq f(p)$
for all $p \in (a,b)$. A similar argument shows that
$f(p) \leq f(b)$ for all $p \in (a,b)$.
Transitivity ensures $f(a) \leq f(b)$, since $(a,b) \neq \varnothing$.
